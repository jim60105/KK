            ...
            foreach (var ext in copyList) {
                switch (ext) {
                    case "KKABMPlugin.ABMData":
                        ...
                        //清理舊角色數據，將身體數據刪除
                        List<object> previousModifiers = new List<object>();
                        foreach (string boneName in (IEnumerable<string>)BoneController.GetType().InvokeMember("GetAllPossibleBoneNames", publicFlag, null, BoneController, null)) {
                            if (null != BoneController.GetType().InvokeMember("GetModifier", publicFlag, null, BoneController, new object[] { boneName })) {
                                previousModifiers.Add(BoneController.GetType().InvokeMember("GetModifier", publicFlag, null, BoneController, new object[] { boneName }));
                            }
                        }
                        int i = 0;
                        foreach (var modifier in previousModifiers.Where(x => !(bool)x.GetType().InvokeMember("IsCoordinateSpecific", publicFlag, null, x, null))) {
                            Logger.Log(LogLevel.Debug, "[KK_SCLO] Clean previous ABMX BoneData: " + (string)modifier.GetType().InvokeMember("BoneName", BindingFlags.GetProperty, null, modifier, null));
                        ...
                        Logger.Log(LogLevel.Debug, $"[KK_SCOLB] Clean {i}/{previousModifiers.Count} previous ABMX Body Modifiers");

                        //將擴充資料由暫存複製到角色身上
                        ExtendedSave.SetExtendedDataById(ocichar.charInfo.chaFile, ext, ExtendedSave.GetExtendedDataById(tmpChaFile, ext));

                        //把擴充資料載入ABMX插件
                        BoneController.GetType().InvokeMember("OnReload", BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Instance | BindingFlags.InvokeMethod, null, BoneController, new object[] { 2, false });

                        //清理新角色數據，將衣服數據刪除
                        List<object> newModifiers = new List<object>();
                        foreach (string boneName in (IEnumerable<string>)BoneController.GetType().InvokeMember("GetAllPossibleBoneNames", publicFlag, null, BoneController, null)) {
                            if (null != BoneController.GetType().InvokeMember("GetModifier", publicFlag, null, BoneController, new object[] { boneName })) {
                                newModifiers.Add(BoneController.GetType().InvokeMember("GetModifier", publicFlag, null, BoneController, new object[] { boneName }));
                            }
                        }
                        i = 0;
                        foreach (var modifier in newModifiers.Where(x => (bool)x.GetType().InvokeMember("IsCoordinateSpecific", publicFlag, null, x, null))) {
                            Logger.Log(LogLevel.Debug, "[KK_SCLO] Clean new ABMX BoneData: " + (string)modifier.GetType().InvokeMember("BoneName", BindingFlags.GetProperty, null, modifier, null));
                            var y = modifier.GetType().InvokeMember("GetModifier", publicFlag, null, modifier, new object[] { (ChaFileDefine.CoordinateType)ocichar.charInfo.fileStatus.coordinateType });
                            y.GetType().InvokeMember("Clear", publicFlag, null, y, null);
                            i++;
                        }
                        Logger.Log(LogLevel.Debug, $"[KK_SCOLB] Clean {i}/{newModifiers.Count} new ABMX Coordinates Modifiers");

                        //將舊的衣服數據合併回到角色身上
                        i = 0;
                        foreach (var modifier in previousModifiers.Where(x => (bool)x.GetType().InvokeMember("IsCoordinateSpecific", publicFlag, null, x, null))) {
                            Logger.Log(LogLevel.Debug, "[KK_SCLO] Merge previous ABMX BoneData: " + (string)modifier.GetType().InvokeMember("BoneName", BindingFlags.GetProperty, null, modifier, null));
                            var y = BoneController.GetType().InvokeMember("AddModifier", publicFlag, null, BoneController, new object[] { modifier });
                            i++;
                        }
                        ...
                        //重整,只是作為保險(?
                        BoneController.SetPrivateProperty("NeedsFullRefresh", true);
                        BoneController.GetType().InvokeMember("LateUpdate", BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Instance | BindingFlags.InvokeMethod, null, BoneController, null);

                        //列出角色身上所有ABMX數據
                        foreach (string boneName in (IEnumerable<string>)BoneController.GetType().InvokeMember("GetAllPossibleBoneNames", publicFlag, null, BoneController, null)) {
                            var m = BoneController.GetType().InvokeMember("GetModifier", publicFlag, null, BoneController, new object[] { boneName });
                            if (null != m) {
                                Logger.Log(LogLevel.Debug, "[KK_SCLO] ABMX BoneData: " + (string)m.GetType().InvokeMember("BoneName", BindingFlags.GetProperty, null, m, null));
                            }
                        }
